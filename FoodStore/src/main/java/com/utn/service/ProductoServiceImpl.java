...
import com.utn.dao.ProductoDAO;
...
public class ProductoServiceImpl implements ProductoService {
    ...
    public void crear(Producto p) {
        if (p.getNombre() == null || p.getNombre().trim().isBlank()) {
            throw new NegocioException("NEGOCIO: El nombre del producto es mandatorio.");
        }
        if (p.getPrecio() == null || p.getPrecio() <= 0) {
            throw new NegocioException("NEGOCIO: El precio debe ser un valor positivo mayor a cero.");
        }
        if (p.getStock() < 0) {
            throw new NegocioException("NEGOCIO: El stock disponible inicial no puede ser negativo.");
        }
        if (p.getCategoria() == null) {
            throw new NegocioException("NEGOCIO: Todo producto debe estar estrictamente vinculado a una Categoría.");
        }
        productoDAO.crear(p);
    }
    ...
    public void editar(Long id, Producto nuevosDatos) {
        ...
        if (prodExistente == null) {
            throw new NegocioException("NEGOCIO: El producto especificado no fue localizado.");
        }
    ...
    public void eliminar(Long id) {
        ...
        if (prod == null) {
            throw new NegocioException("NEGOCIO: Producto inexistente.");
        }
