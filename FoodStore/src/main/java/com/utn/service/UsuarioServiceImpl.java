...
import com.utn.exceptions.NegocioException;
...
public class UsuarioServiceImpl implements UsuarioService {
    ...
    public List<Usuario> listar() {
        ...
        if (listaCompleta == null || listaCompleta.isEmpty()) {
            throw new NegocioException("No hay usuarios registrados en el sistema.");
        }
        ...
        if (activos.isEmpty()) {
            throw new NegocioException("No hay usuarios activos para mostrar.");
        }
    ...
    public void crear(Usuario u, String mail) {
        if (u.getNombre() == null || u.getNombre().trim().isBlank()) {
            throw new NegocioException("NEGOCIO: El nombre del usuario es obligatorio.");
        }
        if (u.getApellido() == null || u.getApellido().trim().isBlank()) {
            throw new NegocioException("NEGOCIO: El apellido del usuario es obligatorio.");
        }
        if (u.getMail() == null || u.getMail().trim().isBlank()) {
            throw new NegocioException("NEGOCIO: El mail del usuario es obligatorio.");
        } else {
            if (validarMailUnico(mail)) {
                throw new NegocioException("NEGOCIO: El mail del usuario ya existe y debe ser único.");
            }
        }
        if (u.getCelular() == null || u.getCelular().trim().isBlank()) {
            throw new NegocioException("NEGOCIO: El celular del usuario es obligatorio.");
        }
        if (u.getContraseña() == null || u.getContraseña().trim().isBlank()) {
            throw new NegocioException("NEGOCIO: La contraseña es obligatoria.");
        }
    ...
    public void editar(int id, Usuario nuevosDatos) {
        ...
        if (usuarioExistente == null || usuarioExistente.isEliminado()) {
            throw new NegocioException("NEGOCIO: Usuario no encontrado o ya fue eliminado.");
        }
    ...
    public void eliminar(int id) {
        ...
        if (usuario == null || usuario.isEliminado()) {
            throw new NegocioException("NEGOCIO: No se encontró un usuario activo con el ID: " + id);
        }
